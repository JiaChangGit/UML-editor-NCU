...
public final class CanvasPanelHandler extends PanelHandler {

    Vector<JPanel> members;
    Vector<JPanel> selectComp;
    int boundShift = 10;
    private final Vector<JPanel> lines;
    ...
    private ILinePainter[] getLineByObject(JPanel basicObject, int port) {
        ...
        for (JPanel line : lines) {
            logger.log(Level.INFO, "check line: {0}", line);
            ...
            if (isInside(members.elementAt(i), e.getPoint()) == true
                ...
                switch (core.isFuncComponent(members.elementAt(i))) {
                    case 0 -> {
                        ...
                        logger.log(Level.INFO, "click on item location: {0}", onItem);
                        logger.log(Level.INFO, "click on port: {0}", port);
